$(document).ready(function(){
    ...
    function dohvatiPodatke()
    {
        var dataTable = $('#tablica').DataTable({
            ...
            "order" : [],
            "ajax" : {
                url:"http://localhost/admin/regije",
                type:"POST"
            }
        });
    }
    ...
    $(document).on('click', '#insert', function(){
        ...
        var kratica = $('#data3').text();
        if(id != '' && naziv != '')
        {
            $.ajax({
                url:"http://localhost/admin/regije/dodaj",
                method:"POST",
                data:{id:id, naziv:naziv, kratica:kratica},
    ...
    $(document).on('click', '.delete', function(){
        ...
        {
            $.ajax({
                url:"http://localhost/admin/regije/obrisi",
                method:"POST",
                data:{id:id},
                success:function(data){
                    $('#poruka').html('<div class="alert alert-success">'+data+'</div>');
    ...
    {
        $.ajax({
            url:"http://localhost/admin/regije/azuriraj",
            method:"POST",
