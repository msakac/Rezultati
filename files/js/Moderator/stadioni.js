$(document).ready(function(){
    ...
    function dohvatiPodatke()
    {
        var dataTable = $('#tablica').DataTable({
            ...
            "order" : [],
            "ajax" : {
                url:"http://localhost/moderator/stadioni",
                type:"POST"
            }
        });
    }
    ...
    $(document).on('click', '#insert', function(){
        ...
        var mjesto = $('#data3').text();
        if(naziv != '' && mjesto!='')
        {
            $.ajax({
                url:"http://localhost/moderator/stadioni/dodaj",
                method:"POST",
                data:{naziv:naziv, mjesto:mjesto},
    ...
    $(document).on('click', '.delete', function(){
        ...
        {
            $.ajax({
                url:"http://localhost/moderator/stadioni/obrisi",
                method:"POST",
                data:{id:id},
                success:function(data){
                    $('#poruka').html('<div class="alert alert-success">'+data+'</div>');
    ...
    {
        $.ajax({
            url:"http://localhost/moderator/stadioni/azuriraj",
            method:"POST",
